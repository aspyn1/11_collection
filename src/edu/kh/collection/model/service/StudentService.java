...
public class StudentService {
	...
	// ArrayList(용량) : 용량만큼의 리스트 생성 -> 마찬가지로 큰 의미는 없다.
	...
	// 저장되는 타입이 Student로 제한된 리스트를 생성했다는 뜻
	...
	public String addStudent() throws InputMismatchException{
		...
		if(studentList.add( new Student(name, age, region, gender, score) ) ) {
			// add의 반환형은 boolean 타입
			// 제네릭 <Student>때문에 List타입이 Student로 제한됨
...
//		}
		...
		// 향상된 for문(for each문)
		// - 컬렉션, 배열의 모든 요소를 순차적으로 반복접근할 수 있은 for문
	...
	public String updateStudent() throws InputMismatchException{
		...
		// -> (반환) Student == 변경 전 Student 객체가 담겨있다.
		...
		// 1) 학생 정보가 studentList에 있는가? == 비어있는가
		if(studentList.isEmpty()) {
		...
		// 4) 입력받은 숫자가 studentList 범위 내 인덱스 번호인가?	
			...
			// 입력받은 index 번째에 새로운 학생 정보를 셋팅 -> 수정
...
//		for(int i = 0; i < studentList.size(); i++) {
//			if(input == studentList.get(i).getName()) {
